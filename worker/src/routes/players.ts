...
export async function handlePlayers(request: Request, env: Env, user: JWTPayload): Promise<Response> {
  ...
  if (request.method === 'GET') {
    ...
    const query = includeInactive
      ? `SELECT id, name, alias, email, phone, role, active, birth_date, shirt_number, license_number, avatar_url, last_seen, COALESCE(notify_email,1) as notify_email, COALESCE(notify_push,1) as notify_push FROM players ORDER BY active DESC, CASE WHEN shirt_number IS NULL THEN 1 ELSE 0 END, shirt_number`
      : `SELECT id, name, alias, email, phone, role, active, birth_date, shirt_number, license_number, avatar_url, last_seen, COALESCE(notify_email,1) as notify_email, COALESCE(notify_push,1) as notify_push FROM players WHERE active=1 ORDER BY CASE WHEN shirt_number IS NULL THEN 1 ELSE 0 END, shirt_number`;
